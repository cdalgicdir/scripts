...
for a in ${anglist[@]}; do
    echo ang_${a} | g_angle -f -n bonded.ndx -type angle -od ang_${a}.xvg -ov time-ang_${a}.xvg
done

for d in ${dihlist[@]}; do
    echo dih_${d} | g_angle -f -n bonded.ndx -type dihedral -od dih_${d}.xvg -ov time-dih_${d}.xvg
done

for i in ${implist[@]}; do
    echo imp_${i} | g_angle -f -n bonded.ndx -type improper -od imp_${i}.xvg -ov time-imp_${i}.xvg
done
