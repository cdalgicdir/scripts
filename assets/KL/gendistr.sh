...
for i in `seq 0 $((${#bondlist[@]}-1))`; do
    b=${bondlist[$i]}
    r=${bondRlist[$i]}
    echo bond_${b} | g_bond -f ../mapped.xtc -s -n bonded.ndx -o bond_${b}.xvg -d time-bond_${b}.xvg -blen $r
done

for a in ${anglist[@]}; do
    echo ang_${a} | g_angle -f ../mapped.xtc -n bonded.ndx -type angle -od ang_${a}.xvg -ov time-ang_${a}.xvg -all
    g_analyze -f time-ang_${a}.xvg -dist hist_ang_${a}.xvg
done

for d in ${dihlist[@]}; do
    echo dih_${d} | g_angle -f ../mapped.xtc -n bonded.ndx -type dihedral -od dih_${d}.xvg -ov time-dih_${d}.xvg -all
    g_analyze -f time-dih_${d}.xvg -dist hist_dih_${d}.xvg
done

for i in ${implist[@]}; do
    echo imp_${i} | g_angle -f ../mapped.xtc -n bonded.ndx -type improper -od imp_${i}.xvg -ov time-imp_${i}.xvg -all
