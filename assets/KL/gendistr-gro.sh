...
for i in `seq 0 $((${#bondlist[@]}-1))`; do
	...
	b=${bondlist[$i]}
	r=${bondRlist[$i]}
	echo bond_${b}-$j | g_bond -f $trj -s $tpr -n $ndx -o bond_${b}-$j.xvg -d time_bond_${b}-$j.xvg -blen $r
...
for i in `seq 0 $((${#bondlist[@]}-1))`; do
    b=${bondlist[$i]}
    r=${bondRlist[$i]}
    echo bond_${b} | g_bond -f $trj -s $tpr -n $ndx -o bond_${b}_avg.xvg -d time_bond_${b}.xvg -blen $r
...
for i in `seq 1 11`; do
    echo dist-CNi-CNi4-${i}a dist-CNi-CNi4-${i}b | g_dist -f $trj -s $tpr -n $ndx -o time_dist_CNi-CNi4-$i
done
